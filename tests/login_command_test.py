...
class LoginCommand_should(unittest.TestCase):
    ...
    def test_should_login_successfully(self):
        result = self.command.execute(["Uasim17022", "123456789"])
        self.assertEqual(result, "Employee Uasim17022 successfully logged in!")
        self.assertEqual(self.app_data.logged_in_employee.username, "Uasim17022")
    ...
    def test_should_raise_error_if_no_username_or_password_provided(self):
        ...
        with self.assertRaises(ValueError):
            self.command.execute(["Uasim17022"])

        with self.assertRaises(ValueError):
            self.command.execute(["Uasim17022", "123456789", "extra_param"])

    def test_should_raise_error_if_wrong_password(self):
        with self.assertRaises(ValueError):
            self.command.execute(["Uasim17022", "wrongpassword"])
    ...
    def test_should_raise_error_if_user_already_logged_in(self):
        self.command.execute(["Uasim17022", "123456789"])
        with self.assertRaises(ValueError):
            self.command.execute(["Uasim17022", "123456789"])
